...
const roi = cam.get(
  ...
  // F) Apply model + material + spawn point (use AB midpoint if possible)
if (three.ready && objects.length > 0) {
  ...
  if (three.current) {
    ...
    // ② 计算 spawn：优先 AB 中点；没有 B 就用 A 中心
    const A = objects[0].rect;
    ...
    // ✅ 3D debug marker（显示原始 hit）
    if (spawnMarker3D) {
      spawnMarker3D.visible = !!hit;
      if (hit) spawnMarker3D.position.copy(hit);
    }
